...
#include "define/BillBoardRenderDesc.h"
#include "define/LightDesc.h"
#include "define/MeshDesc.h"
#include "define/OctreeRenderDesc.h"
#include "define/SkyBoxRenderDesc.h"
#include "define\BasicShapeRenderDesc.h"
#include "define/RectangleRenderDesc.h"
#include "define/SpriteRenderDesc.h"
#include "define/SpriteRectRenderDesc.h"
#include "define/NumberSpriteRenderDesc.h"
#include "define/StringRenderDesc.h"
#include "define/Type.h"
#include "define/ViewportDesc.h"
#include "define/Ray.h"
...
void Renderer::initialize()
{
	...
	perspective(45.0f, 800 / 600, 0.3f, 1000.0f);
	ViewportDesc view;
	view.height = 600;
	view.width = 800;
	viewport(view);
...
void Renderer::render(const BasicShapeRenderDesc & desc)
{
	//透視変換行列の設定
	glMatrixMode(GL_PROJECTION);
	glPushMatrix();
	...
	glLightfv(GL_LIGHT0, GL_POSITION, lightPosition);
	glEnable(GL_LIGHT0);
	glMultMatrixf(desc.mat);
	glMaterialfv(GL_FRONT, GL_DIFFUSE,desc.color);
	//取りあえずの高速化 後で直す
	static SphereMesh mesh = SphereMesh();
	//static TorusMesh mesh = TorusMesh();
	/*static BasicShapeMesh mesh(0,0);
	switch (desc.type)
	{
	case BASICSHAPE_TYPE::SPHERE:
		mesh=SphereMesh();
		break;
	case BASICSHAPE_TYPE::TORUS:
		mesh=TorusMesh();
		break;
	case BASICSHAPE_TYPE::CYLINDER:
		mesh= CylinderMesh();
		break;
	case BASICSHAPE_TYPE::BOX:
		mesh= BoxMesh();
		break;
	}*/
	mesh.draw();
